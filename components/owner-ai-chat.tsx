...
export function OwnerAiChat() {
  ...
  async function ask(event: FormEvent) { event.preventDefault(); if (!message.trim()) return; setLoading(true); setError(''); const supabase = createClient(); const { data, error: invokeError } = await supabase.functions.invoke('ai-owner-chat', { body: { mode: 'chat', message } }); if (invokeError || data?.error) setError(data?.error ?? await messageFor(invokeError)); else setAnswer(data.answer); setLoading(false); }
  async function brief() { setLoading(true); setError(''); const supabase = createClient(); const { data, error: invokeError } = await supabase.functions.invoke('ai-owner-chat', { body: { mode: 'daily_brief' } }); if (invokeError || data?.error) setError(data?.error ?? await messageFor(invokeError)); else setAnswer(data.answer); setLoading(false); }

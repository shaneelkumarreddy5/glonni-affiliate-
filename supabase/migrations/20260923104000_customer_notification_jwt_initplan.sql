...
drop policy "customers and verified staff read notifications" on public.customer_notifications;
create policy "customers and verified staff read notifications" on public.customer_notifications
for select to authenticated
using (
  profile_id=(select auth.uid())
  or (((select auth.jwt()) ->> 'aal')='aal2' and exists(select 1 from public.profiles p join public.employees e on e.profile_id=p.id where p.id=(select auth.uid()) and p.role in ('owner','admin','editor') and e.status='active'))
);

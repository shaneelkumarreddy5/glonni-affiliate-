...
export default { fetch: withSupabase({ auth: ["publishable"] }, async (req, ctx) => {
  ...
  try {
    ...
    const context = JSON.stringify({categories:categories??[],connected_stores:scopedMerchants,pending_work:work??[],owner_instructions:instructions??[],reviewer_instruction:job.input?.reviewer_instruction??null});

...
export default async function Admin({searchParams}:{searchParams:Promise<SearchState>}) {
  ...
  return <main className="admin-v2"><AdminSidebar/><section className="admin-main"><main className="admin-content stores-admin-page">
    <div className="admin-title stores-title"><div><p>STORE OPERATIONS</p><h1>Stores &amp; Brands</h1><span>Open a store to manage its products, categories, cashback, financials, policies and health in one workspace.</span></div><a className="add-store" href="#new-store"><Plus/> Add store</a></div>
    ...
    </article><aside className="stores-sidepanels"><article><header><h2>Store health</h2><Link href="/admin?status=needs-attention">Review stores <ArrowRight/></Link></header><p><i className="health-dot good"/><span>Active<small>Customer-ready stores</small></span><b>{active}</b></p><p><i className="health-dot warn"/><span>Attention<small>Needs store-level review</small></span><b>{attention}</b></p><p><i className="health-dot neutral"/><span>Paused<small>Hidden from shoppers</small></span><b>{all.filter(x=>!x.is_active).length}</b></p></article><article><header><h2>Recently updated stores</h2></header>{(events??[]).map(event=>{const merchant=all.find(x=>x.id===event.entity_id);return merchant?<Link className="sync-event" href={`/admin/stores/${merchant.slug}?tab=overview`} key={event.id}><i className="health-dot good"/><span><b>{merchant.name}</b><small>{event.event_type.replaceAll('_',' ')}</small></span><time>{new Date(event.created_at).toLocaleDateString('en-IN')}</time></Link>:null})}{!(events??[]).length&&<p className="empty-admin">No store changes recorded yet.</p>}</article></aside></section>

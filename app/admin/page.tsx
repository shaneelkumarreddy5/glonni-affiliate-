...
export default async function Admin({searchParams}:{searchParams:Promise<SearchState>}) {
  ...
  return <main className="admin-v2"><AdminSidebar/><section className="admin-main"><main className="admin-content stores-admin-page">
    ...
    <section className="stores-workspace"><article className="stores-directory">
      <div className="store-tabs"><nav><Link className={!query.status||query.status==='all'?'active':''} href="/admin">All stores <b>{all.length}</b></Link><Link className={query.status==='active'?'active':''} href="/admin?status=active">Active <b>{active}</b></Link><Link className={query.status==='paused'?'active':''} href="/admin?status=paused">Paused <b>{all.filter(x=>x.approval_status==='paused').length}</b></Link><Link className={query.status==='needs-attention'?'active':''} href="/admin?status=needs-attention">Needs attention <b>{attention}</b></Link></nav><form><input type="hidden" name="status" value={query.status||'all'}/><Search/><input name="q" defaultValue={query.q} placeholder="Search stores or brands…"/><button>Search</button></form></div>

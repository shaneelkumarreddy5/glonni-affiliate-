...
export async function saveStoreCommissionRule(f: FormData) {
  const { s, user } = await storeOperator();
  const merchantId=String(f.get("merchantId")??""),slugValue=String(f.get("slug")??""),ruleId=String(f.get("ruleId")??""),categoryId=String(f.get("categoryId")??""),providerId=String(f.get("providerId")??"")||null;
  const providerCommission=Number(f.get("providerCommission")),customerCashback=Number(f.get("customerCashback")),cashbackCap=String(f.get("cashbackCap")??"").trim(),effectiveFrom=String(f.get("effectiveFrom")??""),effectiveTo=String(f.get("effectiveTo")??"")||null,status=String(f.get("status")??"draft"),appliesToDescendants=f.get("appliesToDescendants")==="on",exclusions=String(f.get("exclusions")??"").trim()||null;
  if(!merchantId||!slugValue||!categoryId||!effectiveFrom)throw new Error("Store, category and effective date are required.");
  if(!Number.isFinite(providerCommission)||providerCommission<0||providerCommission>100)throw new Error("Enter a valid provider commission from 0% to 100%.");
  if(!Number.isFinite(customerCashback)||customerCashback<0||customerCashback>providerCommission)throw new Error("Customer cashback cannot exceed the provider commission.");
  if(!["draft","scheduled","active","paused","expired"].includes(status))throw new Error("Choose a valid rule status.");
  const {data:validCategory}=await s.from("categories").select("id").eq("id",categoryId).eq("is_active",true).maybeSingle();if(!validCategory)throw new Error("Choose an active catalogue category.");
  if(providerId){const{data:linkedOffer}=await s.from("offers").select("id").eq("merchant_id",merchantId).eq("provider_id",providerId).limit(1).maybeSingle();if(!linkedOffer)throw new Error("This affiliate provider is not connected to the selected store.");}
  const values={merchant_id:merchantId,category_id:categoryId,provider_id:providerId,provider_commission_percent:providerCommission,customer_cashback_percent:customerCashback,cashback_cap:cashbackCap?Number(cashbackCap):null,applies_to_descendants:appliesToDescendants,exclusions,effective_from:effectiveFrom,effective_to:effectiveTo,status,updated_by:user.id,updated_at:new Date().toISOString()};
  const result=ruleId?await s.from("store_commission_rules").update(values).eq("id",ruleId).eq("merchant_id",merchantId):await s.from("store_commission_rules").insert({...values,created_by:user.id});
  if(result.error)throw new Error(result.error.message);
  if(status==="active"){
    const {data:categoryRows}=await s.from("categories").select("id,parent_id");
    const categoryIds=new Set([categoryId]);
    if(appliesToDescendants){let changed=true;while(changed){changed=false;for(const row of categoryRows??[])if(row.parent_id&&categoryIds.has(row.parent_id)&&!categoryIds.has(row.id)){categoryIds.add(row.id);changed=true}}}
    const {data:productRows}=await s.from("products").select("id").in("category_id",[...categoryIds]);
    if(productRows?.length){let offerQuery=s.from("offers").update({commission_rate:providerCommission,reward_type:customerCashback>0?"percentage_cashback":"none",cashback_percent:customerCashback||null,cashback_cap:cashbackCap?Number(cashbackCap):null,cashback_tracking_supported:customerCashback>0,reward_funding_source:customerCashback>0?"provider":"none",updated_at:new Date().toISOString()}).eq("merchant_id",merchantId).in("product_id",productRows.map(x=>x.id));if(providerId)offerQuery=offerQuery.eq("provider_id",providerId);const{error}=await offerQuery;if(error)throw new Error(error.message)}
  }
  await audit(s,"store_commission_rule_saved","merchant",merchantId,{rule_id:ruleId||null,category_id:categoryId,provider_id:providerId,status,actor_id:user.id});
  storeRefresh(slugValue);revalidatePath(`/store/${slugValue}`);redirect(`/admin/stores/${slugValue}?tab=commission&success=Commission%20rule%20saved`);
}
export async function deleteStoreCommissionRule(f:FormData){const{s,user}=await storeOperator();const id=String(f.get("ruleId")??""),merchantId=String(f.get("merchantId")??""),slugValue=String(f.get("slug")??"");if(!id||!merchantId||!slugValue)throw new Error("Commission rule information is missing.");const{error}=await s.from("store_commission_rules").delete().eq("id",id).eq("merchant_id",merchantId);if(error)throw new Error(error.message);await audit(s,"store_commission_rule_deleted","merchant",merchantId,{rule_id:id,actor_id:user.id});storeRefresh(slugValue);redirect(`/admin/stores/${slugValue}?tab=commission&success=Commission%20rule%20deleted`)}
export async function saveStoreProviderPreference(f:FormData){const{s,user}=await storeOperator();const merchantId=String(f.get("merchantId")??""),slugValue=String(f.get("slug")??""),categoryId=String(f.get("categoryId")??"")||null,selectionMode=String(f.get("selectionMode")??"automatic"),preferredProviderId=String(f.get("preferredProviderId")??"")||null;if(!merchantId||!slugValue||!["automatic","prefer_provider","manual_only"].includes(selectionMode))throw new Error("Provider selection information is invalid.");if(selectionMode==="prefer_provider"&&!preferredProviderId)throw new Error("Choose the preferred provider.");if(categoryId){const{data:validCategory}=await s.from("categories").select("id").eq("id",categoryId).eq("is_active",true).maybeSingle();if(!validCategory)throw new Error("Choose an active catalogue category.");}if(preferredProviderId){const{data:linkedOffer}=await s.from("offers").select("id").eq("merchant_id",merchantId).eq("provider_id",preferredProviderId).limit(1).maybeSingle();if(!linkedOffer)throw new Error("This affiliate provider is not connected to the selected store.");}let query=s.from("store_provider_preferences").select("id").eq("merchant_id",merchantId);query=categoryId?query.eq("category_id",categoryId):query.is("category_id",null);const{data:existing}=await query.maybeSingle();const values={merchant_id:merchantId,category_id:categoryId,selection_mode:selectionMode,preferred_provider_id:selectionMode==="prefer_provider"?preferredProviderId:null,updated_by:user.id,updated_at:new Date().toISOString()};const result=existing?await s.from("store_provider_preferences").update(values).eq("id",existing.id):await s.from("store_provider_preferences").insert(values);if(result.error)throw new Error(result.error.message);await audit(s,"store_provider_preference_saved","merchant",merchantId,{category_id:categoryId,selection_mode:selectionMode,preferred_provider_id:preferredProviderId,actor_id:user.id});storeRefresh(slugValue);redirect(`/admin/stores/${slugValue}?tab=health&success=Provider%20selection%20saved`)}

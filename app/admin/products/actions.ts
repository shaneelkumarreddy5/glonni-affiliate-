...
export async function applyAiProductProposal(raw: unknown, existingId?: string) {
  ...
  await mergeManualMetadata(supabase, productId!, { model_code: cleanText(proposal.model_code, 160), canonical_identity: proposal.canonical_identity&&typeof proposal.canonical_identity==="object"?proposal.canonical_identity:{}, workflow_step: "basic", ai_offer_candidates: offerCandidates, ai_image_candidates: Array.isArray(proposal.image_candidates)?proposal.image_candidates.slice(0,12):[], ai_enrichment: { confidence: Math.max(0, Math.min(100, Number(proposal.confidence) || 0)), section_confidence: proposal.section_confidence&&typeof proposal.section_confidence==="object"?proposal.section_confidence:{}, category_reason: cleanText(proposal.category_reason,1000), conflicts, missing_fields: missingFields, duplicate_candidates: duplicateCandidates, sources: sourceRows, model: cleanText(proposal.ai_model, 80), fetched_at: new Date().toISOString(), needs_review: true } });

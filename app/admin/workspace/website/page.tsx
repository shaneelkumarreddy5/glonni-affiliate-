...
import { websitePageOptions, type WebsiteDraftBlock, type WebsiteLayoutSnapshot, type WebsitePageKey } from '@/lib/website-layout';
...
const parseSnapshot = (snapshot: unknown): WebsiteDraftBlock[] => {
  if (!snapshot || typeof snapshot !== 'object') return emptyBlocks;
  const blocks = (snapshot as Partial<WebsiteLayoutSnapshot>).blocks;
  return Array.isArray(blocks) ? blocks as WebsiteDraftBlock[] : emptyBlocks;
};
...
export default async function WebsiteWorkspacePage() {
  ...
  for (const page of websitePageOptions) {
    ...
    publishedLayouts[page.key] = parseSnapshot(row.published_layout);
    const savedDraft = latestVersion.get(row.id)?.snapshot;
    layouts[page.key] = savedDraft ? parseSnapshot(savedDraft) : publishedLayouts[page.key];
  }
  ...
  return <main className="admin-v2"><AdminSidebar/><section className="admin-main website-workspace-main"><header className="admin-top website-top"><b>Website workspace</b><span className="dashboard-date">Build and preview customer pages</span><span className="dashboard-mode">DRAFTS STAY PRIVATE</span></header><main className="admin-content website-workspace-content">
    <div className="admin-title website-page-title"><div><p>WORKSPACE · WEBSITE</p><h1>Website</h1><span>Add and arrange banners and catalogue sections. Core product, category and store layouts stay protected.</span></div><a className="website-open-link" href="/" target="_blank" rel="noreferrer">Open customer site ↗</a></div>
    ...
    <WebsiteWorkspace initialPage="home" initialLayouts={layouts} publishedLayouts={publishedLayouts} pageStatuses={statuses} stores={workspaceStores} products={workspaceProducts} categories={categories.map((category) => ({ id: category.id, name: category.name, slug: category.slug, parentId: category.parent_id }))} canEdit={canEdit}/>
  </main></section></main>;
}

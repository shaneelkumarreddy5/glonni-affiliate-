...
function SelectedOrderList({ items, selectedIds, entityName, onMove }: { items: PickerItem[]; selectedIds: string[]; entityName: string; onMove: (id: string, direction: -1 | 1) => void }) {
  ...
  return <div className={styles.categoryOrder}>
    <b>Display order</b>
    <ol>{selected.map((item, index) => <li key={item.id}><span><i>{index + 1}</i>{item.label}</span><div><button type="button" disabled={index === 0} onClick={() => onMove(item.id, -1)} aria-label={`Move ${item.label} ${entityName} up`} title="Move up">↑</button><button type="button" disabled={index === selected.length - 1} onClick={() => onMove(item.id, 1)} aria-label={`Move ${item.label} ${entityName} down`} title="Move down">↓</button></div></li>)}</ol>
...
export function WebsiteWorkspace({ initialPage, initialLayouts, initialOrders, initialCoreContent, publishedLayouts: initialPublishedLayouts, publishedOrders: initialPublishedOrders, publishedCoreContent: initialPublishedCoreContent, pageStatuses, stores, products, categories, canEdit }: Props) {
  ...
  function moveCoreStore(key: string, id: string, direction: -1 | 1) {
    updateCoreContent(key, { store_ids: moveSelectedId(currentCoreContent[key]?.store_ids ?? [], id, direction) });
  }
  ...
  function moveBlockStore(blockId: string, storeId: string, direction: -1 | 1) {
    updateBlock(blockId, (block) => ({ ...block, config: { ...block.config, store_ids: moveSelectedId(block.config.store_ids ?? [], storeId, direction) } }));
  }
        ...
        {!selected && !selectedCore ? <div className={styles.inspectorEmpty}><LayoutTemplate/><b>Select a section to edit</b><span>Drag any section using its grip, or choose “Add here” to place a new section exactly where you want it.</span><div><b>Pick what each card opens</b><small>Products open product pages, categories open category pages, and stores open store pages.</small></div></div> : selectedCore ? <>
          ...
          <div className={styles.inspectorBody} ref={inspectorBodyRef}>
            ...
            {selected.block_type === 'category_rail' && <>
              ...
              <SelectedOrderList items={allCategoryChoices} selectedIds={selected.config.category_ids ?? []} entityName="category" onMove={(id, direction) => moveBlockCategory(selected.id, id, direction)}/>
            ...
            {selected.block_type === 'store_directory' && <>
              <label>Number of stores<input type="number" min={1} max={50} value={selected.config.count ?? 10} onChange={(event) => updateBlock(selected.id, (block) => ({ ...block, config: { ...block.config, count: Math.max(1, Math.min(50, Number(event.target.value) || 1)) } }))}/></label>
              <div className={styles.sharedCategoryCardNote}><b>Shared store card</b>Each card shows only the store logo and name, and opens that store’s Glonni page. Shape stays consistent wherever store cards appear.</div>
              ...
              <SelectedOrderList items={allStoreChoices} selectedIds={selected.config.store_ids ?? []} entityName="store" onMove={(id, direction) => moveBlockStore(selected.id, id, direction)}/>
              <small className={styles.sourceNote}><Check/> Each card opens its store’s Glonni page. The same store can appear in another section.</small>
            </>}

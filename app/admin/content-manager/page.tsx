...
import { decideContentForAdmin, saveContentDraft, submitContentForCeoReview } from './actions';
...
const TABS = [
  ['overview', 'Overview'], ['recommendations', 'Deal recommendations'], ['create', 'Create draft'], ['drafts', 'Draft library'], ['social', 'Social queue'], ['ads', 'Ads queue'], ['history', 'History'],
] as const;
...
export default async function ContentManagerPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const approvedCount = campaignsList.filter((row) => ['approved', 'admin_approved'].includes(row.status)).length;
  ...
  return <main className="admin-v2"><AdminSidebar/><section className="admin-main"><header className="admin-top"><PenLine size={21}/><b>Content Manager</b><span className="dashboard-date">Campaign creative workspace · approval protected</span><Bell size={19}/><span className="avatar">SR</span></header>
    <main className="admin-content content-manager">
      <header className="cm-head"><div><p>ADS MANAGER / CONTENT STUDIO</p><h1>Content Manager</h1><span>Choose an eligible deal, prepare channel-specific creative, then send it through CEO and admin approval.</span></div><Link className="cm-head-link" href="/admin/ads"><Megaphone size={16}/> Ads Manager</Link></header>
      <div className="cm-flow"><span><BarChart3/>Deal recommendations</span><b>→</b><span><PenLine/>Creative draft</span><b>→</b><span><ShieldCheck/>CEO review</span><b>→</b><span><CheckCircle2/>Final admin approval</span><b>→</b><span><ArrowRight/>Connected channel</span></div>
      ...
      <nav className="cm-tabs" aria-label="Content Manager pages">{TABS.map(([key, label]) => <Link key={key} className={tab === key ? 'active' : ''} href={tabHref(key, key === 'create' ? { offer: query.offer, q: query.q } : {})}>{label}{key === 'drafts' && <b>{draftCount}</b>}{key === 'recommendations' && <b>{recommendations.length}</b>}{key === 'social' && <b>{assetsList.filter((asset) => asset.channel_type === 'social').length}</b>}{key === 'ads' && <b>{assetsList.filter((asset) => asset.channel_type === 'paid_ads').length}</b>}</Link>)}</nav>
      ...
      {tab === 'create' && <section className="cm-create-layout"><article className="cm-panel cm-create-panel"><header><div><p>{selectedCampaign ? 'REVISE CONTENT DRAFT' : 'NEW CREATIVE DRAFT'}</p><h2>{selectedCampaign ? 'Update the returned draft' : 'Prepare content for selected channels'}</h2><span>One deal link and one shared creative for the destinations you choose. Product imagery is not copied; upload your designed creative.</span></div></header><div className="cm-agent-notice"><Sparkles/><span><b>AI content generation is unavailable right now</b><small>Content &amp; Experience Manager is not connected. You can still prepare and save a manual draft.</small></span><Link href="/admin/ai-providers">Provider setup</Link></div>
        ...
        </form></article><aside className="cm-panel cm-create-steps"><h2>What happens next</h2><ol><li><span>1</span><div><b>Save the draft</b><small>Copy and uploaded creative stay private in Content Manager.</small></div></li><li><span>2</span><div><b>Send for CEO review</b><small>The CEO checks the offer facts, copy, image and destination.</small></div></li><li><span>3</span><div><b>Final admin approval</b><small>Only owner/admin can approve the reviewed package.</small></div></li><li><span>4</span><div><b>Route when connected</b><small>Social/ad delivery is unavailable until accounts and APIs are connected.</small></div></li></ol><div className="cm-no-publish"><CircleAlert/><span>Choosing a destination here prepares a draft only. It does not publish a post or create an ad.</span></div></aside></section>}

      {(tab === 'drafts' || tab === 'social' || tab === 'ads') && <section className="cm-panel cm-library"><header><div><p>{tab === 'social' ? 'SOCIAL DESTINATIONS' : tab === 'ads' ? 'PAID AD DESTINATIONS' : 'PRIVATE CONTENT LIBRARY'}</p><h2>{tab === 'social' ? 'Social campaign drafts' : tab === 'ads' ? 'Paid campaign drafts' : 'All campaign drafts'}</h2><span>{tab === 'drafts' ? 'Review the package state and continue the approval workflow.' : 'These are saved creative drafts; connected platform accounts are still required for delivery.'}</span></div><Link className="cm-primary-link" href={tabHref('create', { offer: selectedOffer?.id })}>+ New draft</Link></header>
        {tab === 'drafts' && selectedCampaign && <div className="cm-selected-campaign"><div><small>SELECTED CAMPAIGN</small><h3>{selectedCampaign.title}</h3><p>Status: <b className={`cm-status status-${selectedCampaign.status}`}>{statusLabel(selectedCampaign.status)}</b></p>{selectedCampaign.decision_note && <p>{selectedCampaign.decision_note}</p>}</div>{selectedCampaign.status === 'approved' && <form action={decideContentForAdmin}><input type="hidden" name="campaignId" value={selectedCampaign.id}/><label><input name="note" placeholder="Approval note (optional)"/></label><button name="decision" value="admin_approved"><CheckCircle2 size={15}/> Final admin approve</button><button className="cm-reject" name="decision" value="rejected">Reject</button></form>}</div>}
        {assetsList.filter((asset) => tab === 'drafts' || (tab === 'social' ? asset.channel_type === 'social' : asset.channel_type === 'paid_ads')).length ? <div className="cm-asset-list">{assetsList.filter((asset) => tab === 'drafts' || (tab === 'social' ? asset.channel_type === 'social' : asset.channel_type === 'paid_ads')).map((asset) => { const imageUrl = asset.media_path ? signedMedia.get(asset.media_path) : null; return <article className="cm-asset-card" key={asset.id}><div className="cm-asset-visual">{imageUrl ? <img src={imageUrl} alt="Creative draft"/> : <span><FileImage/><small>No uploaded creative</small></span>}</div><div className="cm-asset-main"><div className="cm-asset-top"><span className={`cm-channel-pill ${asset.channel_type}`}>{asset.channel_type === 'social' ? <PenLine/> : <Megaphone/>}{PLATFORM_NAMES[asset.platform_key] ?? asset.platform_key}</span><b className={`cm-status status-${asset.status}`}>{statusLabel(asset.status)}</b></div><h3>{asset.campaign.title}</h3><p className="cm-asset-headline">{asset.headline || 'No headline added'}</p><p className="cm-asset-copy">{asset.caption || asset.description || 'No customer copy added.'}</p><div className="cm-asset-facts"><span>Format · {asset.creative_format}</span><span>CTA · {asset.cta_label || 'Not set'}</span><a href={asset.destination_url} target="_blank" rel="noreferrer">Tracked deal link <ArrowRight size={13}/></a></div><div className="cm-asset-footer"><Link href={tabHref('drafts', { campaign: asset.campaign_id })}>Campaign details</Link>{['draft', 'on_hold', 'changes_requested'].includes(asset.status) && <Link href={tabHref('create', { offer: asset.campaign.source_offer_id, campaign: asset.campaign_id })}>Edit draft <PenLine size={14}/></Link>}{asset.status === 'draft' && <form action={submitContentForCeoReview}><input type="hidden" name="campaignId" value={asset.campaign_id}/><button>Send campaign to CEO <ArrowRight size={14}/></button></form>}{asset.status === 'on_hold' && <span className="cm-hold-note">CEO requested a re-check. Revise the draft before resubmission.</span>}{asset.status === 'approved' && (asset.campaign.status === 'approved' ? <Link href={tabHref('drafts', { campaign: asset.campaign_id })}>Final admin review <ArrowRight size={14}/></Link> : <span className="cm-hold-note">CEO approved · waiting on other creative reviews</span>)}{asset.status === 'admin_approved' && <span className="cm-hold-note">Approved — waiting for platform connection</span>}</div></div></article>; })}</div> : <div className="cm-empty"><FileImage/><b>Nothing here yet</b><span>Choose an eligible offer, create a draft and select this destination.</span><Link href={tabHref('recommendations')}>Browse deal recommendations <ArrowRight size={14}/></Link></div>}
      </section>}

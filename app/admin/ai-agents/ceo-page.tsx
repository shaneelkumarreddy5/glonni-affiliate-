...
export async function CeoOperationsPage({ searchParams }: { searchParams: Promise<{ tab?: string; agent?: string; success?: string; error?: string }> }) {
  ...
  const [{ data: rows }, { data: work }, { data: instructions }] = await Promise.all([
    supabase.from('ai_agents').select('*').in('key', ['ceo_operations', ...agentCards.map(item => item.key)]),
    supabase.from('ai_work_items').select('id,title,summary,area,risk_level,status,proposed_by,created_at').order('created_at', { ascending: false }).limit(200),
  ...
  return <main className="admin-v2 ai-admin"><AdminSidebar/><section className="admin-main ceo-shell"><main className="ceo-page">
    ...
    <section className="ceo-agent-panel"><header><div><h2>{selected.name}</h2><p>CEO-scrutinized work for this agent only.</p></div><span className="ceo-scope-note">This view is filtered to {selected.name}.</span></header><nav className="ceo-tabs"><a className={tab === 'approvals' ? 'active' : ''} href={panelHref(selected.key, 'approvals')}>Approvals <b>{selectedWork.length}</b></a><a className={tab === 'instructions' ? 'active' : ''} href={panelHref(selected.key, 'instructions')}>Instructions <b>{instructionText.length}</b></a></nav>
      {tab === 'approvals' ? <div className="ceo-approval-layout"><div className="ceo-approval-list"><div className="ceo-panel-title"><h3>{selected.name} approvals</h3><span>Only work that has reached CEO review is shown.</span></div>{selectedWork.length ? selectedWork.map(item => <article className="ceo-approval-row" key={item.id}><div><b>{item.title}</b><p>{item.summary}</p><small>{item.proposed_by} · {formatTime(item.created_at)}</small></div><em className={`risk-${item.risk_level}`}>{item.risk_level}</em><strong>{item.status.replaceAll('_', ' ')}</strong><div className="ceo-row-actions"><form action={decideAiWork}><input type="hidden" name="workItemId" value={item.id}/><input type="hidden" name="decision" value="approved"/><input type="hidden" name="note" value={`Approved by CEO for ${selected.name} admin review.`}/><button className="approve">Approve for admin</button></form><form action={decideAiWork}><input type="hidden" name="workItemId" value={item.id}/><input type="hidden" name="decision" value="on_hold"/><input type="hidden" name="note" value="Request agent re-check and correction."/><button>Re-check</button></form><form action={decideAiWork}><input type="hidden" name="workItemId" value={item.id}/><input type="hidden" name="decision" value="rejected"/><input type="hidden" name="note" value="Rejected by CEO."/><button className="reject">Reject</button></form></div></article>) : <div className="ceo-empty"><CheckCircle2/><b>No pending approvals for this agent</b><span>New CEO-reviewed work will appear here when this agent submits it.</span></div>}</div><aside className="ceo-review-card"><h3>CEO review rule</h3><p>The agent may prepare work automatically. The CEO checks its evidence and instructions before forwarding anything to admin approval.</p><div><span>Selected agent</span><b>{selected.name}</b></div><div><span>Connected pages</span><b>Products, categories and store pages</b></div></aside></div> : <div className="ceo-instruction-layout"><div className="ceo-instruction-list"><div className="ceo-panel-title"><h3>{selected.name} instructions</h3><span>Active rules used by this agent.</span></div>{instructionText.map((text, index) => <article key={`${text}-${index}`}><span>{index + 1}</span><p>{text}</p><small>{allInstructionText.includes(text) ? 'Saved instruction' : 'Default instruction'}</small></article>)}<form className="ceo-add-instruction" action={addOwnerInstruction}><input type="hidden" name="scope" value={selected.scope}/><input type="hidden" name="redirectTo" value={panelHref(selected.key, 'instructions')}/><textarea name="instruction" required minLength={3} placeholder="Add an instruction for this agent…"/><div><select name="schedule"><option value="active">Active now</option><option value="scheduled">Schedule later</option></select><button>Save instruction</button></div></form></div><aside className="ceo-review-card"><h3>How instructions work</h3><p>Only active instructions are sent to the selected agent. The CEO can add rules now and schedule date-based work later.</p><ul><li>Agent follows these rules</li><li>CEO reviews its result</li><li>Admin gives final approval</li></ul></aside></div>}
    </section>

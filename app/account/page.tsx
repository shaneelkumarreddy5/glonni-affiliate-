...
import { BellRing, ChevronRight, CircleHelp, Headphones, Heart, Landmark, MapPin, ReceiptText, ShieldCheck, SlidersHorizontal, UserRound, WalletCards } from 'lucide-react';
...
const sections = [
  ...
  ['wallet', 'Wallet', 'Cashback balance and reward history', WalletCards],
  ['payouts', 'Withdraw & Payouts', 'KYC, payout method and withdrawals', Landmark],
...
export default async function AccountPage({ searchParams }: Props) {
  ...
  return <><Header/><main className="account-hub">
    ...
    <div className="account-hub-layout">
      ...
      <section className="account-workspace">{active === 'profile' && <ProfileSection name={name} profile={profile} email={user.email || ''}/>} {active === 'wallet' && <WalletSection available={available} pending={pending}/>} {active === 'payouts' && <PayoutSection/>} {active === 'shopping' && <ShoppingSection saved={saved}/>} {active === 'preferences' && <PreferencesSection preferences={preferences}/>} {active === 'help' && <HelpSection/>}</section>
    </div>
  </main></>;
}
...
function ProfileSection({ name, profile, email }: { name: string; profile: { city: string | null; state: string | null } | null; email: string }) { return <><WorkspaceTitle eyebrow="PROFILE" title="Your personal details" body="Keep your profile accurate so Glonni can personalise discovery and support."/><form action={updateProfile} className="profile-form" encType="multipart/form-data"><label className="avatar-upload">Profile photo<input name="avatar" type="file" accept="image/jpeg,image/png,image/webp"/><small>JPG, PNG or WebP · up to 2 MB</small></label><label>Your name<input name="displayName" required minLength={2} maxLength={80} defaultValue={name}/></label><label>City<input name="city" maxLength={80} defaultValue={profile?.city || ''} placeholder="Your city"/></label><label>State<input name="state" maxLength={80} defaultValue={profile?.state || ''} placeholder="Your state"/></label><label>Email address<input value={email} disabled/></label><button type="submit">Save profile</button></form><section className="security-note"><ShieldCheck/><div><b>Account security</b><span>Your sign-in session stays on this device until you sign out. Password and account-security changes will be added here.</span></div></section><form action={signOut}><button className="quiet-action" type="submit">Sign out</button></form></>; }
function WalletSection({ available, pending }: { available: number; pending: number }) { return <><WorkspaceTitle eyebrow="WALLET" title="Your cashback wallet" body="Only provider-confirmed cashback becomes available for withdrawal."/><div className="wallet-balance"><div><small>Available cashback</small><b>{money(available)}</b><span>Confirmed and eligible</span></div><Link href="/wallet">Open wallet <ChevronRight size={16}/></Link></div><div className="workspace-cards"><Link href="/wallet"><ReceiptText/><span><b>Cashback history</b><small>View confirmed credits and reversals.</small></span><ChevronRight size={16}/></Link><Link href="/cashback-claim"><BellRing/><span><b>Missing cashback claims</b><small>{money(pending)} currently in review.</small></span><ChevronRight size={16}/></Link></div></>; }
function PayoutSection() { return <><WorkspaceTitle eyebrow="WITHDRAW & PAYOUTS" title="Prepare for withdrawals" body="KYC and a payout method are required before any confirmed cashback can be withdrawn."/><div className="workspace-cards"><article><ShieldCheck/><span><b>KYC verification</b><small>Not started. Identity verification will open when production payout verification is enabled.</small></span><em>Not started</em></article><article><Landmark/><span><b>Bank account or UPI</b><small>Add and verify a payout method from your Wallet when withdrawals are enabled.</small></span><Link href="/wallet">Open wallet <ChevronRight size={16}/></Link></article><Link href="/wallet"><ReceiptText/><span><b>Withdrawal history</b><small>View request status and completed payout records.</small></span><ChevronRight size={16}/></Link></div></>; }

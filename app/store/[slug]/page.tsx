...
export default async function StorePage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<StoreFilters> }) {
  ...
  const storeSections: Record<string, ReactNode> = {
    'core:store_intro': <section className="store-hero store-profile-hero"><p className="eyebrow">SHOP BY STORE</p><div>{store.logo_url ? <span className="store-hero-logo"><img src={store.logo_url} alt=""/></span> : <span>{store.name.slice(0, 1)}</span>}<section><h1>{renderWebsiteRichText(coreContent.store_intro?.title || `${store.name} on Glonni`)}</h1><p>{coreContent.store_intro?.body ? renderWebsiteRichText(coreContent.store_intro.body) : `Browse products available from ${store.name}, then open a product to compare this store with every other connected seller.`}</p></section></div><aside><b>{new Set(allOffers.map((offer) => offer.products?.id).filter(Boolean)).size}</b><small>products</small><b>{allOffers.length}</b><small>offers</small><b>{rewards}</b><small>cashback-eligible</small></aside></section>,
    'core:store_products': <section className="vertical-section store-products-section">
      <div className="section-title"><div><p className="eyebrow">{store.name.toUpperCase()} PRODUCTS</p><h2>{renderWebsiteRichText(coreContent.store_products?.title || 'Browse and compare')}</h2>{coreContent.store_products?.body && <span>{renderWebsiteRichText(coreContent.store_products.body)}</span>}</div>{hasFilters && <Link className="clear-category-filters" href={`/store/${store.slug}?from=${encodeURIComponent(returnPath)}`}><RotateCcw size={14}/>Clear filters</Link>}</div>

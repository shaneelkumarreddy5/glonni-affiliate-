...
export default async function SupportConversation({params,searchParams}:{params:Promise<{ticketId:string}>;searchParams:Promise<{error?:string;success?:string}>}){
  ...
  const[{data:ticketData},{data:messageData}]=await Promise.all([supabase.from('support_tickets').select('id,ticket_number,subject,category,status,support_state,source_channel,created_at,updated_at,resolution_note').eq('id',ticketId).eq('profile_id',user.id).maybeSingle(),supabase.from('support_messages').select('id,author_type,body,created_at').eq('ticket_id',ticketId).eq('visibility','customer').order('created_at')]);
  ...
  return <><Header/><main className="support-layout"><BrowseNav items={[{label:'Help & Legal',href:'/account?section=help'},{label:'My support requests',href:'/support/requests'},{label:`Request #${ticket.ticket_number}`}]} fallback="/support/requests"/><SupportChannelNav active="requests"/><div className="support-workspace" style={{display:'block'}}><section className="ticket-detail">
    ...
    <header className="ticket-detail-head"><div><p>REQUEST #{ticket.ticket_number} · {ticket.source_channel}</p><h1>{ticket.subject}</h1><span>{ticket.category.replaceAll('_',' ')} · opened {new Date(ticket.created_at).toLocaleDateString('en-IN',{day:'numeric',month:'short',year:'numeric'})}</span></div><em className={ticket.support_state}>{ticket.support_state.replaceAll('_',' ')}</em></header>

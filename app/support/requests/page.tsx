...
export default async function SupportRequestsPage({searchParams}:{searchParams:Promise<{error?:string;success?:string}>}){
  ...
  const{data}=await supabase.from('support_tickets').select('id,ticket_number,subject,category,status,updated_at').eq('profile_id',user.id).order('updated_at',{ascending:false}).limit(50);const tickets=(data??[])as Ticket[];
  return <><Header/><main className="support-layout"><BrowseNav items={[{label:'Profile',href:'/account?section=help'},{label:'Help & Legal',href:'/account?section=help'},{label:'My support requests'}]} fallback="/account?section=help"/><div className="support-workspace" style={{display:'block'}}><section className="requests-page">

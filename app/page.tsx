...
export default async function Home() {
  ...
  const homeSections: Record<string, ReactNode> = {
    ...
    'core:best_deals': <section id="deals"><div className="section-title"><div><p className="eyebrow">BEST DEALS</p><h2>{renderWebsiteRichText(coreContent.best_deals?.title || 'Best deals right now')}</h2>{coreContent.best_deals?.body && <span className="home-managed-copy">{renderWebsiteRichText(coreContent.best_deals.body)}</span>}</div><a href="/deals?sort=best">View all deals</a></div><HomeOfferRail offers={selectedProducts('best_deals', bestDeals)} bestDeal shape={coreContent.best_deals?.visual_shape}/></section>,
    ...
    'core:price_drops': <section><div className="section-title"><div><p className="eyebrow">PRICE DROPS</p><h2>{renderWebsiteRichText(coreContent.price_drops?.title || 'Worth a closer look')}</h2>{coreContent.price_drops?.body && <span className="home-managed-copy">{renderWebsiteRichText(coreContent.price_drops.body)}</span>}</div><a href="/deals?sort=price-drop">View all price drops</a></div><HomeOfferRail offers={selectedProducts('price_drops', priceDrops)} shape={coreContent.price_drops?.visual_shape}/></section>,

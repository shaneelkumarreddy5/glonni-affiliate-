...
export default async function Home() {
  ...
  const homeSections: Record<string, ReactNode> = {
    ...
    'core:categories': <section id="categories" className="home-category-anchor" aria-labelledby="home-categories-title">
      <div className="section-title"><div><p className="eyebrow">BROWSE CATEGORIES</p><h2 id="home-categories-title">{categoryContent.title || 'What are you shopping for?'}</h2>{categoryContent.body && <span className="home-managed-copy">{renderWebsiteRichText(categoryContent.body)}</span>}</div></div>
    ...
    'core:best_deals': <section id="deals"><div className="section-title"><div><p className="eyebrow">BEST DEALS</p><h2>{coreContent.best_deals?.title || 'Best deals right now'}</h2>{coreContent.best_deals?.body && <span className="home-managed-copy">{renderWebsiteRichText(coreContent.best_deals.body)}</span>}</div><a href="/deals?sort=best">View all deals</a></div><HomeOfferRail offers={selectedProducts('best_deals', bestDeals)} bestDeal shape={coreContent.best_deals?.visual_shape}/></section>,
    'core:trending': <section><div className="section-title"><div><p className="eyebrow">TRENDING NOW</p><h2>{coreContent.trending?.title || 'Popular picks across stores'}</h2>{coreContent.trending?.body && <span className="home-managed-copy">{renderWebsiteRichText(coreContent.trending.body)}</span>}</div><a href="/deals?sort=trending">View all trending deals</a></div><HomeOfferRail offers={selectedProducts('trending', trending)} shape={coreContent.trending?.visual_shape}/></section>,
    'core:price_drops': <section><div className="section-title"><div><p className="eyebrow">PRICE DROPS</p><h2>{coreContent.price_drops?.title || 'Worth a closer look'}</h2>{coreContent.price_drops?.body && <span className="home-managed-copy">{renderWebsiteRichText(coreContent.price_drops.body)}</span>}</div><a href="/deals?sort=price-drop">View all price drops</a></div><HomeOfferRail offers={selectedProducts('price_drops', priceDrops)} shape={coreContent.price_drops?.visual_shape}/></section>,
    'core:benefits': <section className="benefits">{(coreContent.benefits?.title || coreContent.benefits?.body) && <div className="section-title"><div><h2>{coreContent.benefits?.title || 'Why shop with Glonni?'}</h2>{coreContent.benefits?.body && <span className="home-managed-copy">{renderWebsiteRichText(coreContent.benefits.body)}</span>}</div></div>}

...
const outstanding = ['requested', 'on_hold', 'approved', 'batched', 'processing'];
...
function Status({ value }: { value: string }) { const label = value.replaceAll('_', ' '); const tone = ['confirmed', 'paid'].includes(value) ? 'confirmed' : ['submitted', 'requested', 'approved', 'batched', 'processing'].includes(value) ? 'pending' : value === 'on_hold' ? 'hold' : ['rejected', 'reversed', 'failed'].includes(value) ? 'rejected' : 'neutral'; return <span className={`wallet-status ${tone}`}>{label}</span>; }

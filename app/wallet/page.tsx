...
export default async function WalletPage({ searchParams }: Props) {
  ...
  return <><Header/><main className="info-page"><section className="info-hero"><p className="eyebrow">MY SPACE · CASHBACK &amp; WALLET</p><h1>Your Glonni Wallet</h1><p>Only provider-confirmed cashback is available to withdraw. Requests are manually reviewed; no automatic money transfer is active.</p></section>
    ...
    <section className="info-content"><h2>Request a withdrawal</h2><form action={requestWithdrawal} className="support-form"><label>Amount available to withdraw<input name="amount" type="number" min="100" max={available} step="0.01" required placeholder="Minimum ₹100"/></label><label>UPI ID<input name="upiId" required maxLength={100} placeholder="name@bank"/></label><small>We verify payout details during manual review. Your request does not move money automatically.</small><button className="primary" type="submit">Request withdrawal</button></form>
